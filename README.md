...
2. **정밀 질의응답**: LangGraph 기반의 AI 에이전트가 특정 기간·날짜·범위를 추출하고, 질문과 관련된 복수 쿼리를 생성하여 검색 정확도를 높입니다. DB에 데이터가 있는 경우 특정 기간·날짜·범위 질의, 기간 비교 분석, 추이 요약 등에 강점을 보입니다. 
...
## DB 범위
- 출처: KCIF 국제금융센터 (`https://www.kcif.or.kr`)
- 수집 기간: 2023년 1월 21일 ~ 현재
- 수집 주기: `python setup.py` 실행 시 영업일 기준 최신 데이터 자동 누적
- 수집 항목: 국제금융속보 뉴스 (환율, 금리, 주가, NDF, CDS 등 국제금융시장 지표 및 국내외 금융 이슈)
...
- CSV 데이터(`data/kcif_articles_accumulate.csv`)와 Vector DB(`data/chroma_db_bge/`)는 `data/` 프로젝트 루트에 있어야 하며, 없을 경우 `setup.py` 실행 시 자동 생성(데이터 최초 수집 및 DB 생성 시 네트워크 환경에 따라 시간이 다소 소요될 수 있습니다.)
